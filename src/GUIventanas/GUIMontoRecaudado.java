...
import GUIcontroladores.ControladorMontoRecaudado;
import GUIcontroladores.ControladorRegistrarAlumno;

//import GUIcontroladores.


public class GUIMontoRecaudado {

	private JFrame frame;
	private JPanel contentPanel;
	
	private ControladorMontoRecaudado controlador;
	...
	private void initialize(JPanel cardPanelP, CardLayout cardLayoutP, JFrame frameP) {
		...
		contentPanel.setLayout(null);
		controlador = new ControladorMontoRecaudado(this);
		...
		JLabel tituloReq1 = new JLabel("MONTO");
		...
		JLabel tituloReq2 = new JLabel("RECAUDADO");
		...
		JLabel labelCampo1 = new JLabel("Año lectivo:");
		...
		JTextField anioInput = new JTextField();
		anioInput.setFont(new Font("Courier New", Font.PLAIN, 15));
		anioInput.setColumns(35);
		anioInput.setBorder(new LineBorder(Color.WHITE, 5));
		anioInput.setPreferredSize(new Dimension(35, 25));
		panelDatos.add(anioInput);
		...
		panelOutput.setBackground(new Color(246, 240, 229));
		panelOutput.setBounds(20, 400, 350, 90);
		contentPanel.add(panelOutput);
		panelOutput.setLayout(new FlowLayout(FlowLayout.LEFT, 10, 15));
	
		JLabel labelCampo3 = new JLabel("Monto recaudado: ");
		labelCampo3.setFont(new Font("Courier New", Font.PLAIN, 15));
		labelCampo2.setHorizontalAlignment(SwingConstants.LEFT);
		labelCampo2.setFont(new Font("Courier New", Font.PLAIN, 15));
		panelOutput.add(labelCampo3);
		
		JTextArea montoOutput = new JTextArea();
		montoOutput.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
		montoOutput.setEditable(false);
		montoOutput.setText("$$$");
		montoOutput.setFont(new Font("Courier New", Font.PLAIN, 15));
		montoOutput.setBorder(new LineBorder(Color.WHITE, 5));
		montoOutput.setColumns(35);
		montoOutput.setPreferredSize(new Dimension(35, 25));
		panelOutput.add(montoOutput);
		...
		buttonAceptar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String anioS = anioInput.getText();
				String cedulaS = cedulaInput.getText();
				montoOutput.setText("$$$");
				montoOutput.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
				
				if(anioS.isBlank() || cedulaS.isBlank()) {
				...
				try {
					...
					if(anio < 0) {
						GUIMensaje mensaje = new GUIMensaje(frame, "El año no puede ser menor a 0.", true);
						return;
					}
				...
				double monto = -1;
				monto = controlador.recaudarMonto(anio, cedula);
				if(monto != -1.0) {
					montoOutput.setText(monto + "");
					montoOutput.setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
				}
			}
		});
	}
	
	public GUIMontoRecaudado(JPanel cardPanelP, CardLayout cardLayoutP, JFrame frameP) {
